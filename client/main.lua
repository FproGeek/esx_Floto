...
function OpenLoterieMenu()
	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'participer',
		{
			title    = "Loterie",
			elements = {
				{label = "Participer au tirage (5000$)", value = 'participer -'}
			}
		},

		function(data, menu)
			TriggerServerEvent("elec:participer",data.current.value)
			menu.close()
		end,
		function(data, menu)
			menu.close()
	...
	while true do
		...
		for k,v in pairs(Config.Zones) do
			if(GetDistanceBetweenCoords(coords, v.MarkerPos.x, v.MarkerPos.y, v.MarkerPos.z, true) < v.Size.x) then
				drawTxt('Utiliser ~g~E~s~ pour ~b~participer au tirage', 2, 1, 0.5, 0.8, 0.6, 255, 255, 255, 255)
